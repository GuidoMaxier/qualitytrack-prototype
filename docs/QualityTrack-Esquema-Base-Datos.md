...
|---|---|---|---|
| NO-Country · Semana 1 de 5 | 03/09/2026 | Consolidado — pendiente de validación técnica | `spec/QualityTrack-PRD.md`, `spec/QualityTrack-Backlog-v1.md` |

Esta versión unifica las dos propuestas independientes de esquema (v1 de Mel Zarate y v1 de Ángel) y aplica las correcciones surgidas de contrastarlas contra el PRD y el Backlog. Los cambios que este trabajo obliga a hacer en esos dos documentos están listados aparte, en `spec/QualityTrack-Cambios-PRD-Backlog.md`.
...
erDiagram
    ...
    USUARIOS {
        bigint id PK
        varchar nombre
        varchar email "nullable para operarios"
        varchar password_hash "nullable para operarios"
        varchar rol
        varchar tipo_tarea
        varchar telefono
        boolean activo
...
| `tipo_tarea` | VARCHAR(100) | SÍ | | Ej. `Tornero`, `Soldador` | Solo para operarios (HU-5.2) |
...
| `fecha_esperada_entrega` | DATE | NO | | | Fecha que pide el cliente. Informativa (D3) |
...
**R3 · Competencia del operario.** `ot_fases.operario_id` debe corresponder a un usuario con `rol = 'OPERARIO'` que tenga un registro con `habilitado = TRUE` en `fase_operarios_habilitados` para el mismo `fase_catalogo_id`. Cruza dos tablas con condición dinámica: se valida en la aplicación, con trigger opcional (incluido comentado en el script). *(HU-5.1.)*
...
**R6 · Sin FKs redundantes en la OT.** La OT se vincula únicamente a la cotización. Solicitud, cliente y vendedor se alcanzan navegando. Evita que tres caminos al mismo dato puedan contradecirse. *(HU-1.2 depende de que ese recorrido sea consistente.)*
...
**R10 · Checklist completo antes del veredicto.** Cada auditoría debe tener las 7 respuestas cargadas y **ninguna con `resultado_item` nulo** antes de confirmar el resultado. SQL no puede exigir "exactamente N filas hijas, todas respondidas" sin trigger: se valida en la aplicación. *(HU-4.2.)*
...
-- Regla R3 (operario habilitado para la fase) — trigger opcional.
-- Descomentar si se prefiere garantizarla en la base y no solo en la aplicación.
--
